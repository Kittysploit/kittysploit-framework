...
class Cve(Base):
    ...
    def __str__(self):
        return f"Cve_id: {self.cve_id}, Summary: {self.summary}, Cvss2: {self.cvss2}, Cvss3: {self.cvss3}"
...
if not os.path.exists(base_path() + "/database"):
    os.mkdir(base_path() + "/database")

engine = create_engine(
    f"sqlite:///{base_path()}/database/kittyDatabase.db?check_same_thread=False",
    echo=False,
)
