...
class Client_api:
	...
	def __init__(self, ip="127.0.0.1", port=5555, user="kitty", password="kitty"):
		self.loading = 0
		self.token = ""
		self.ip = ip
		self.port = port
	...
	def start(self):
		...
		print("")
		print("+ -- --=[ Welcome")
		print("+ -- --=[ Kittysploit framework api session client")
		...
		while True:
			...
			if command == "exit":
				...
				try:
					get_sessions = requests.get(f"http://{self.ip}:{self.port}/api/sessions", headers=headers, timeout=5)
				except requests.exceptions.ConnectionError:
				...
				if check["success"] == True:
					...
					while True:
						...
						if c == "exit":
							...
							try:
								r = requests.get(f"http://{self.ip}:{self.port}/api/interact?id_session={id_session}&cmd={c}", headers=headers, timeout=8)
							except requests.exceptions.ConnectionError:
